...
--◆◆◆ 부족했던 부분 : between and 연산자가 아니라 in( , )연산자 사용해서 표현하기 ◆◆◆ 
...
--◆◆◆ 부족했던 부분 : 문제파악 오류.. 관리자가 없는 사원의 경우 mgr칼럼에서 찾기 ◆◆◆ 
--10. 관리자가 없는 사원의 이름과 담당 업무를 출력하시오.
select ename, job 
from emp
where mgr is null; --MGR가 null값을 가지는 사원 찾기
...
--◆◆◆ 부족했던 부분 : '%A%E%' 처럼 순서 없이 and를 사용하여 모두 포함하고 있는 조건 표현하기 ◆◆◆ 
--13. 이름에 A와 E를 모두 포함하고 있는 사원의 이름을 표시하시오.
select ename
from emp
--where ename like '%A%E%' or ename like '%E%A%';  이렇게 하는 것보다 아래처럼 수정하기
...
where (job ='CLERK' or job= 'SALESMAN') and sal not in(1600, 950, 1300);  --job or 부분은 괄호 필요
...
select ename, sal, comm
from emp
where comm>=500;  --comm이 null이면 아예 연산의 대상이 되지 않기 때문에, 결과에는 반영되지 않는다.
