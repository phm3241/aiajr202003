		...
		$.ajax({
			...
			success: function(data){
				...
				for(var i=0; i<data.length; i++){
					...
					html += '   <span>'+label+'</span><span id="'+data[i].iidx+'" class="'+alarm+'" onclick="cancleAlarm('+data[i].iidx+','+data[i].buyer+', seller)"></span>';
	...
	function selectBuyer_ok(iidx, oidxArr, midxArr){
		...
		$.ajax({
			...
			success : function(data){
				...
				for(var i=0; i<midxArr.length(); i++){
					sendMessage(loginMidx, midxArr[i], iidx);
				}
				midxArr = [];
		...
		if(confirm('선택하신 참여자의 참여를 거절하시겠습니까?')){
			$.ajax({
				...
				success : function(data){
					...
					sendMessage(loginMidx, midx, iidx);
				},
				error:function(jqXHR, textStatus, errorThrown){
					alert("에러 발생~~ \n" + textStatus + " : " + errorThrown);
				}
		...
		$.ajax({
			...
			success : function(data){
				...
				for(var i=0; i<rejectMidxArr.length(); i++){
					sendMessage(loginMidx, rejectMidxArr[i], iidx);
				}
				rejectMidxArr = [];
			},
			error:function(jqXHR, textStatus, errorThrown){
